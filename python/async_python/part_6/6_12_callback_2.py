...
async def get_codding_message(code: str):
    return code


def decode_message(task: asyncio.Task):
    code = task.result()
    if int(code[-1], base=16) % 2 == 0:
        msg = "Неверный код, сообщение скрыто"
    else:
...
async def main():
    tasks = []
    for code in codes:
        task = asyncio.create_task(get_codding_message(code))
        task.add_done_callback(decode_message)
        tasks.append(task)
