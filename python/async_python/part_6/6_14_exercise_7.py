...
class MessageCorrector:
    ...
    def moderator(self):
        for banned_word in banned_words:
            self.msg = self.msg.replace(banned_word, "****")
        return self.msg
    ...
    def student(self):
        for banned_word in banned_words:
            if banned_word in self.msg:
                return "В сообщении есть запрещённое слово, сообщение скрыто"
        return self.msg

    def black_list_user(self):
        return "Пользователь забанен, сообщение скрыто"


async def check_message(msg: str):
    current_task = asyncio.current_task()
    role = current_task.get_name()
    corrector = MessageCorrector(msg)
    try:
        print(f"{role}: {getattr(corrector, role)()}")
...
async def main():
    await asyncio.gather(
        *[asyncio.create_task(check_message(msg["message"]), name=msg["role"])
          for msg in msgs])
