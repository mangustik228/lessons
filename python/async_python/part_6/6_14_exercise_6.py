...
banned_words = {'ошибка', 'баг', 'отладка', 'память', 'процессор',
                'компиляция', 'алгоритм', 'функция', 'база данных', 'интерфейс'}
...
async def check_message(msg: dict[str, str]):
    words = [''.join(filter(str.isalpha, word.strip().lower()))
...
async def main():
    ...
    for msg, result in zip(msgs, results):
        if isinstance(result, asyncio.exceptions.CancelledError):
            print(
                f'В сообщении {msg["message_id"]} стоп-слово: task.done(): False')
        else:
